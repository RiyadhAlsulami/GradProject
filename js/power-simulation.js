...
// Simulation settings
const SIMULATION_INTERVAL = 2000; // Update every  seconds (reduced from 30 seconds)
...
async function updateDeviceConsumption(device) {
    ...
    // Skip updates for devices that are turned off
    ...
    try {
        // Always update consumption, even if device is off
        // This ensures values increase for testing purposes
...
window.PowerSimulation = {
    ...
    forceUpdate: async () => {
        ...
        // Add spinning animation to icon if elements exist
        ...
        if (!simulationRunning) {
            ...
            // Reload devices on the dashboard after updating
            if (window.location.pathname.includes('dashboard.html')) {
                // Get current user
                const { data: sessionData } = await supabase.auth.getSession();
                if (sessionData.session) {
                    const user = sessionData.session.user;
                    // Call loadDevices function from dashboard.js
        ...
        // Remove spinning animation if simulation was already running
        if (updateIcon && updateBtn) {
            updateIcon.classList.remove('icon-spin');
            updateBtn.disabled = false;
        }
